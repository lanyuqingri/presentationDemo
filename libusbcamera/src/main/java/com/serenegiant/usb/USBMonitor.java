...
public final class USBMonitor {
	...
	private static final String TAG = "rokid_glass_tag";
...
//			if (DEBUG) Log.i(TAG, "unregister:");
	...
	public int getDeviceCount() throws IllegalStateException {
		if (destroyed) throw new IllegalStateException("already destroyed");
		return getDeviceList(mDeviceFilters).size();
	}
...
//		}
		...
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					if (DEBUG) Log.v(TAG, "requestPermission   &&& call onConnect if app already has permission");
					// call onConnect if app already has permission
					processConnect(device);
				} else {
					try {
						if (DEBUG) Log.v(TAG, "requestPermission   &&& mPermissionIntent: " + mPermissionIntent.toString());
				...
				if (DEBUG) Log.v(TAG, "isRegistered: but device is null");
				processCancel(device);
				result = true;
			}
		} else {
			if (DEBUG) Log.v(TAG, "isRegistered: false");
			processCancel(device);
			result = true;
		}
		return result;
	...
	private final Runnable mDeviceCheckRunnable = new Runnable() {
		...
		public void run() {
			if (destroyed) return;
			final List<UsbDevice> devices = getDeviceList(mDeviceFilters);
